...
- [ ] `dotnet restore .\shadowsocks-reborn.sln -p:Platform=x64 -r win-x64` succeeds.
- [ ] `dotnet build .\shadowsocks-reborn.sln -c Release -p:Platform=x64 -m:1 --no-restore` succeeds.
- [ ] `dotnet test .\Shadowsocks.UnitTests\Shadowsocks.UnitTests.csproj -c Release -p:Platform=x64 --no-build` succeeds.
- [ ] `dotnet restore .\Shadowsocks.UI\Shadowsocks.UI.csproj -p:Platform=x64 -p:PublishProfile=FolderProfile -r win-x64` succeeds before any `publish --no-restore`.
...
Open the ZIP and confirm that `shadowsocks-reborn.exe` and `Shadowsocks.NetworkService.exe` are both present. The package must not contain helper `.dll`, `.deps.json`, `.runtimeconfig.json`, Privoxy, sysproxy or legacy native-crypto binaries.

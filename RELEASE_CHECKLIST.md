...
- [ ] `dotnet restore .\shadowsocks-windows.sln -p:Platform=x64 -r win-x64` succeeds.
...
- [ ] `dotnet restore .\shadowsocks-csharp\shadowsocks-csharp.csproj -p:Platform=x64 -p:PublishProfile=FolderProfile -r win-x64` succeeds before any `publish --no-restore`.

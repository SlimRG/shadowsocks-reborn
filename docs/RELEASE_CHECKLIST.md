...
- [ ] Stress DNSCrypt crash/restart and shared local-port scenarios, including a host with TCP/UDP dynamic or excluded-port differences; the allocated DNSCrypt loopback port must be usable by both transports. FLOW-layer PID attribution identifies new endpoints and ambiguous ownership falls back without guessing a PID.

...
## Текущий релиз: 5.2.22

Релиз 5.2.22 завершает текущий этап интеграции DNSCrypt/DoH при отключённом ODoH, исправляет блокировавшие релиз регрессии updater/tests/localization, защищает staged self-updater на границе UAC и усиливает CI/release gates для актуального .NET servicing baseline, NuGet vulnerability audit, совместимости с Windows 10 build 19041 и версии опубликованного EXE. Страница логов использует актуальный выделяемый `RichTextBlock`; старые `ListView`-инварианты больше не являются частью UI-контракта.

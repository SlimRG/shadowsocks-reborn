...
The Plugins page installs managed plugin packages below `Plugins\<plugin-id>` in the active storage root. Built-in catalog entries download a Windows x64 release archive; manual installation accepts ZIP and TAR.GZ packages. Server configuration persists the SIP003 plugin id, while runtime resolution maps that id to the installed executable. Legacy absolute-path/PATH plugin values remain supported as a fallback.

Because `PluginsDirectory` is derived from `StorageRoot`, normal mode uses `%LOCALAPPDATA%\Shadowsocks\Plugins` and Clean Mode uses the current `%TEMP%\Shadowsocks\Clean\...\Plugins` session automatically. Clean Mode plugin packages therefore disappear with the rest of the session on Quit.

    ...
    public static class Utils
    {
        ...
        public static string GetTempPath()
        {
            if (_tempPath == null)
            {
                try
                {
                    string instance = Math.Abs(GetDeterministicHashCode(Shadowsocks.Core.RuntimeEnvironment.ExecutablePath)).ToString("X8");
                    _tempPath = Directory.CreateDirectory(Path.Combine(AppStoragePaths.TempWorkingRoot, instance)).FullName;
                }
                catch (Exception e)
                {
                    logger.Error(e);

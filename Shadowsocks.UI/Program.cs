...
namespace Shadowsocks
{
    internal static class Program
    {
        ...
        private static readonly Mutex mutex = new Mutex(true, $"shadowsocks-reborn_{Utils.GetDeterministicHashCode(ExecutablePath)}");
        ...
        private static void Main(string[] args)
        {
            ...
            if (hasAnotherInstance)
            {
                ...
                {
                    MessageBox.Show(I18N.GetString("Find shadowsocks-reborn icon in your notify tray.")
                                    + Environment.NewLine
                                    + I18N.GetString("If you want to start multiple shadowsocks-reborn, make a copy in another directory."),
                        I18N.GetString("shadowsocks-reborn is already running."));
                }
                return;
            }
            #endregion
            ...
            RuntimeEnvironment.Initialize(ExecutablePath, WorkingDirectory, Args);
            ...
            MainController = new ShadowsocksController(new Shadowsocks.UI.WinFormsUserInteractionService());
            MenuController = new MenuViewController(MainController);

            HotKeys.Init(MainController);
            HotkeyReg.Initialize(MainController);
        ...
        private static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            if (Interlocked.Increment(ref exited) == 1)
            {
                ...
                logger.Error(errMsg);
                MessageBox.Show(
                    $"{I18N.GetString("Unexpected error, shadowsocks-reborn will exit. Please report to")} {UpdateChecker.IssuesUrl} {Environment.NewLine}{errMsg}",
                    "shadowsocks-reborn non-UI Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Application.Exit();
            }
        }
        ...
        private static void Application_ThreadException(object sender, ThreadExceptionEventArgs e)
        {
            if (Interlocked.Increment(ref exited) == 1)
            {
                ...
                MessageBox.Show(
                    $"{I18N.GetString("Unexpected error, shadowsocks-reborn will exit. Please report to")} {UpdateChecker.IssuesUrl} {Environment.NewLine}{errorMsg}",
                    "shadowsocks-reborn UI Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Application.Exit();
            }
        }
